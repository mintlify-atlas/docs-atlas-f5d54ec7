...
return (
  ...
  >
    ...
    <div onPointerDown={(e) => { if (!isMaximized) dragControls.start(e); }}>
...
`WindowContext.js` declares a module-level variable `let zIndex = 10` outside any React component. Every call to `focusWindow(id)` executes:

```js
zIndex += 1;
// then updates the matching window object with the new zIndex value
```

Because the variable lives at module scope it persists across re-renders and across calls to any of the context actions. The result is a simple monotonically increasing counter: each newly focused window gets a higher z-index than every other window, so the most recently touched window is always on top. New windows created by `openWindow` also increment `zIndex` and apply it immediately.
